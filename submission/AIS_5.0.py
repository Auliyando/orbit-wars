...
class KaggleAISBallisticSubmissionAgent:
    def __init__(self):
        # --- BASELINE EXTRAPOLATION COEFFICIENTS ---
        # These constants dictate the slopes of your dynamic parametric functions
        self.alpha_dis = 0.0419
        self.beta_dis = 0.0935
        self.alpha_gr = 1.2462
        self.alpha_ns = 0.0782
        self.gamma_pool = 0.7135
        self.alpha_pool = 0.0348
        self.beta_pool = 0.5
    ...
    def execute_macro_turn(self, obs):
        try:
            ...
            # -----------------------------------------------------------------
            # CONTINUOUS PARAMETRIC CONTEXT RESOLUTION ENGINE
            # -----------------------------------------------------------------
            # Metric 1: Player Scale Factor (P)
            max_owner_id = max([p[1] for p in normalized_planets] + [0])
            P = 2.0 if max_owner_id <= 1 else 4.0
            
            # Metric 2: Map Sparsity Scale (S)
            ...
            # Metric 3: Endgame Progression Ratio (phi)
            ...
            # Execution of Dynamic Mathematical Heuristics
            ...
            moves = []
            if is_early_game:
                # --- PHASE A: COOPERATIVE SYNCHRONIZED POOLING ---
