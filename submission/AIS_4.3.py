...
class KaggleAISBallisticSubmissionAgent:
    def __init__(self):
        # AIS Evolutionary Optimized Coefficients (GA-Tuned Output Vectors)
        self.W_NS = 0.5836           # Garrison Penalty: How much the bot avoids heavily defended targets
        ...
        # --- PERSISTENT EVENT-DRIVEN STATE REGISTRIES ---
    ...
    def execute_macro_turn(self, obs):
        try:
            ...
            # =====================================================================
            # PREDICTIVE AIS TARGET SELECTION LOOP (CACHE-AWARE)
            # =====================================================================
            ...
            for target in hostile_planets:
                ...
                # --- STATE CACHE INTERCEPT: PRUNE PURSUED NODES ---
                ...
                if intercept_data[0] is not None:
                    ...
                    if target[1] != -1: 
                        predicted_enemy_garrison += target[6] * (math.ceil(f_time) + 1) # Added +1 arrival buffer
            ...
            # =====================================================================
            # FLEET DISPATCH PROTOCOL LAYER (STRIKE TEAM CONSENSUS SYSTEM)
            # =====================================================================
            moves = []
            
            if is_early_game:
                # --- PHASE A: DISTRIBUTED SYNCHRONIZED POOLING GATE ---
                # Discover baseline required threshold to secure conquest
                ...
                # First Pass: Assemble strike team members with clear lines of sight
                ...
                for my_p in my_planets:
                    ...
                    # Verify this specific outpost has an unblocked trajectory past celestials
                ...
                # Consensus Gate: Only deploy fleets if team can completely clear the contract
                if total_unblocked_surplus >= final_needed:
                    ...
                    for my_p, angle, safe_surplus in strike_team_candidates:
                        ...
                        # Apply your strict min-5 guardrail contract
                        if launch_ships < 5:
                            if safe_surplus >= 5: launch_ships = 5
                            else: continue
                        
                        # Re-verify trajectory vector mapping for targeted launch mass
                    ...
                    # Lock target in virtual blacklist to prevent multi-turn trickle spamming
