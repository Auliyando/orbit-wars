...
def check_planet_collision(sx, sy, tx, ty, obs, source_id, target_id):
    ...
    # Extract and normalize current planet states from observation packet
    ...
    for p in normalized_planets:
        ...
        # Skip the source base and intended target to prevent false positives
        if p_id == source_id or p_id == target_id:
            continue
            
        # FIXED: Changed 'cy' to 'py' to correctly map the target line vector segment projection
        t = max(0.0, min(1.0, ((px - sx) * dx + (py - sy) * dy) / len_sq))
        
        # Find the coordinates of the closest point on the flight vector
        closest_x = sx + t * dx
        closest_y = sy + t * dy
        
        # Calculate Euclidean distance to the obstacle planet's center
        dist = math.sqrt((closest_x - px)**2 + (closest_y - py)**2)
        
        # Enforce collision boundary with a 0.5 ship-hull safety margin
...
def calculate_exact_ballistic_angle(source_planet, target_planet, num_ships, game_angular_velocity, obs):
    ...
    # 10-Step Lookahead Convergence Loop
    for _ in range(10):
    ...
    # Final Kinematics Validation Check
    if check_sun_collision(sx, sy, pred_x, pred_y): return None, 0.0
    # Check if future trajectory collides with any moving sister planet
...
class KaggleAISBallisticSubmissionAgent:
    def __init__(self):
        ...
        self.W_NS = 1.0153           # Garrison Penalty: How much the bot avoids heavily defended targets
        self.W_GR = 0.4018           # Production Reward: How much the bot values permanent income
        self.W_DIS = 1.4553          # Distance Penalty: How much the bot hates long travel times
        self.POOL_PERC = 0.5715
        
        # --- INITIALIZE PERSISTENT EVENT-DRIVEN REGISTRIES ---
    ...
    def execute_macro_turn(self, obs):
        try:
            ...
            # --- YOUR CHOSEN REACTIVE INVALIDATION TRIGGER ---
            # If our planet count changes (gain or lose), completely wipe the virtual pruning cache
            ...
            for target in hostile_planets:
                ...
                # --- YOUR SOLUTION INTERCEPT: PRUNE VIRTUALLY CAPTURED TARGETS ---
                if target[0] in self.virtual_blacklist:
                    continue # Drops planet from consideration entirely until ownership event fires
                    ...
                    if angle is not None:
                        ...
                        if int(my_p[5] - 1) >= final_needed:
                            ...
                            if angle is not None:
                                ...
                                # --- LOCKOUT EVENT MATRIX ---
                                # Secure the target ID in memory to remove it from selection loops
